...
1. 테스트 케이스 T
2. 괄호 문자열을 담을 stack1
3. 문자열 판단 결과를 담을 stack2
4. 테이스 케이스 범위 T 안에서 
   1. 괄호 문자열 v를 입력
   2. 문자열의 길이만큼 for문을 돌린다
      1. 만약 입력할 문자열 v의 i 번째가  ")" 이고 배열이 비어있지 않다면
         1. 배열을 하나 지운다.
      2. 그렇지 않다면 v 의 i 번째를 배열에 추가한다.
   3. for문을 돈 후 배열이 비어있다면 두번째 배열에 YES를 추가한다.
   4. 그렇지 않다면 NO를 추가한다.
   5. 배열을 비운다.
5. 테스트 케이스를 T 만큼 돈 후 T의 범위 안에서 두번째 스택을 차례로 출력한다. 
